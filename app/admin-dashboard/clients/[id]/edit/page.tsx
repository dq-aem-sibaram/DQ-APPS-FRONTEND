...
const EditClientPage = () => {
  ...
  useEffect(() => {
    const fetchClient = async () => {
      ...
      await withLoading(async () => {
        ...
        const wrapper: WebResponseDTOClientDTO =
        ...
        const validTaxDetails =
          dto.clientTaxDetails?.filter((tax) => {
            const isValid =
              tax.taxId &&
            ...
            if (!isValid) {
              console.warn(
                `Skipping invalid tax detail on fetch: "${tax.taxName || "Unnamed"
                }", taxId:`,
                tax.taxId
              );
            }
        ...
        const model: ClientModel = {
          ...
          addresses: dto.addresses?.length
            ? dto.addresses.map((addr) => ({
                ...addr,
                addressId: addr.addressId ?? "", // ← Updated: Preserve existing, empty for new/missing
            ...
            : [
                {
                  addressId: "", // ← Updated: Empty for new/insert
                  houseNo: "",
                  streetName: "",
                  city: "",
                  state: "",
          ...
          clientPocs:
            Array.isArray(dto.pocs) && dto.pocs.length > 0
              ? dto.pocs.map((poc) => ({
                  pocId: poc.pocId ?? "", // ← Updated: Preserve existing, empty for new/missing
                  name: poc.name ?? "",
                  email: poc.email ?? "",
              ...
              : [
                  {
                    pocId: "", // ← Updated: Empty for new/insert
                    name: "",
                    email: "",
                    contactNumber: "",
                    designation: "",
        ...
        if (validTaxDetails.length < (dto.clientTaxDetails?.length || 0)) {
          console.warn(
            `Filtered out ${dto.clientTaxDetails?.length - validTaxDetails.length || 0
            } invalid tax details on load.`
          );
        }
        console.log(
          "Mapped to ClientModel (tax details count):",
          model.clientTaxDetails.length
        );
        setFormData(model);
      }).catch((err) => {
  ...
  const handleChange = (
    ...
    if (["gst", "panNumber", "tanNumber"].includes(name))
      parsed = value.toUpperCase().trim();
  ...
  const removeItem = async (
    ...
    let deleteResult: WebResponseDTOString | null = null;
    switch (section) {
      case "addresses":
        const address = item as AddressModel;
        if (address.addressId && address.addressId !== "") {
          deleteResult = await adminService.deleteEmployeeAddress(
            id as string,
            address.addressId
          );
        }
        break;
      case "clientPocs":
        const poc = item as ClientPocModel;
        if (poc.pocId && poc.pocId !== "") {
          deleteResult = await adminService.deleteClientPocDetails(
            id as string,
            poc.pocId
          );
        }
        break;
      case "clientTaxDetails":
        const tax = item as ClientTaxDetail;
        if (tax.taxId && tax.taxId !== "") {
          deleteResult = await adminService.deleteClientTaxDetails(
            id as string,
            tax.taxId
          );
        }
    ...
    if (deleteResult && !deleteResult.flag) {
      setError(deleteResult.message || `Failed to delete ${section}`);
      return;
    }
    setFormData((prev) => ({
      ...prev!,
  ...
  const confirmAndRemove = async (
    ...
    const sectionName = section === "addresses" ? "address" : section === "clientPocs" ? "POC" : "tax detail";
    const result = await Swal.fire({
      title: 'Are you sure?',
      text: `Do you want to remove this ${sectionName}?`,
      icon: 'warning',
    ...
    if (result.isConfirmed) {
      await removeItem(section, index);
      Swal.fire(
        'Deleted!',
        `${sectionName.charAt(0).toUpperCase() + sectionName.slice(1)} has been deleted.`,
        'success'
      );
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await withLoading(async () => {
      ...
      if (
        ...
        throw new Error(
          "Primary address: city, state, country, pincode required"
        );
      }
      ...
      if (
        !nameRegex.test(formData.companyName) ||
      ...
      if (!phoneRegex.test(formData.contactNumber))
        throw new Error("Contact: 10 digits (6-9 start)");
      if (!["USD", "INR", "EUR"].includes(formData.currency))
      ...
      for (const poc of formData.clientPocs!) {
        if (poc.name && !nameRegex.test(poc.name))
          throw new Error("POC name: letters only");
        if (poc.email && !emailRegex.test(poc.email))
          throw new Error("Invalid POC email");
        if (poc.contactNumber && !phoneRegex.test(poc.contactNumber))
          throw new Error("POC contact: 10 digits");
      }
      // Tax
      ...
      const allClientsWrapper: WebResponseDTOListClientDTO =
      ...
      // Send all: Existing with ID for update, new with empty ID for insert (addresses, POCs, taxes)
      const payload: any = {
        ...formData,
        clientPocs: formData.clientPocs, // ← Use 'clientPocs' to match backend DTO
      };
      console.log("Payload prepared (POC count):", payload.clientPocs.length); // ← Updated log
      console.log("Full payload:", payload);
      // === Update ===
      console.log("Calling updateClient API...");
      const result: WebResponseDTOString = await adminService.updateClient(
        id as string,
        payload
      );
              ...
              <section className="border-b border-gray-200 pb-8">
                ...
                <div className="space-y-6">
                  {formData.clientPocs!.map((poc, i) => (
                    <div
                      key={i}
                      className="p-6 border border-gray-200 rounded-xl bg-gradient-to-r from-gray-50 to-blue-50"
                    >
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-1">
                          <label
                            htmlFor={`clientPocs.${i}.name`}
                            className="block text-sm font-semibold text-gray-700"
                          >
                            POC Name
                          </label>
                        ...
                        <div className="space-y-1">
                          <label
                            htmlFor={`clientPocs.${i}.contactNumber`}
                            className="block text-sm font-semibold text-gray-700"
                          >
                            POC Contact
                          </label>
                        ...
                        <div className="space-y-1">
                          <label
                            htmlFor={`clientPocs.${i}.designation`}
                            className="block text-sm font-semibold text-gray-700"
                          >
                            POC Designation
                          </label>
                      ...
                      {formData.clientPocs!.length > 1 && (
                        <button
                          type="button"
                          onClick={() => confirmAndRemove("clientPocs", i)}
                ...
                <div className="space-y-6">
                  {formData.addresses!.map((addr, i) => (
                    <div
                      ...
                      className="p-6 border border-gray-200 rounded-xl bg-gradient-to-r from-gray-50 to-purple-50"
                    >
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        <div className="space-y-1">
                          <label
                            htmlFor={`addresses.${i}.houseNo`}
                            className="block text-sm font-semibold text-gray-700"
                          >
                            House No
                          </label>
                        ...
                        <div className="space-y-1">
                          <label
                            htmlFor={`addresses.${i}.streetName`}
                            className="block text-sm font-semibold text-gray-700"
                          >
                            Street Name
                          </label>
                        ...
                        <div className="space-y-1">
                          <label
                            htmlFor={`addresses.${i}.addressType`}
                            className="block text-sm font-semibold text-gray-700"
                          >
                            Address Type
                          </label>
                      ...
                      {formData.addresses!.length > 1 && (
                        <button
                          type="button"
                          onClick={() => confirmAndRemove("addresses", i)}
                          className="mt-4 bg-red-100 text-red-700 px-4 py-2 rounded-xl hover:bg-red-200 transition-all text-sm font-semibold"
                        >
                          Remove Address
                        </button>
              ...
              <section className="pb-8">
                ...
                <div className="space-y-4">
                  {formData.clientTaxDetails.map((tax, i) => (
                    <div
                      key={tax.taxId}
                      className="p-6 border border-gray-200 rounded-xl bg-gradient-to-r from-gray-50 to-orange-50 flex gap-6 items-start"
                    >
                      <div className="flex-1 space-y-1">
                        <label
                          htmlFor={`clientTaxDetails.${i}.taxName`}
                          className="block text-sm font-semibold text-gray-700"
                        >
                          Tax Name
                        </label>
                      ...
                      <div className="w-32 space-y-1">
                        <label
                          htmlFor={`clientTaxDetails.${i}.taxPercentage`}
                          className="block text-sm font-semibold text-gray-700"
                        >
                          Percentage (%)
                        </label>
                      ...
                      {formData.clientTaxDetails.length > 1 && (
                        <button
                          type="button"
                          onClick={() => confirmAndRemove("clientTaxDetails", i)}
